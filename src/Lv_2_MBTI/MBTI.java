...
public class MBTI {
	...
	public void starttest() throws InputException {
		...
		for(int i=0; i<MBTI_VS.length;i++) {
			System.out.println("질문 " +(i+1) + "번 >> "+MBTI_VS[i]);
			if(i==0) {
				System.out.println("1. 집에 혼자 있는 걸 좋아한다.  2. 나가서 사람들과 어울리는 걸 좋아한다");
				System.out.print("1과 2 중 선택 >>");
				int ch = sc.nextInt();
				Pers.put("E/I", ch);
			}
			else if( i==1) {
				System.out.println("1. 엘리베이터는 이동수단일 뿐, 중간에 서지 않고 빨리만 올라갔으면 좋겠다.  2. 사고가 나면 어떻게 탈출을 해야하지?");
				System.out.print("1과 2 중 선택 >>");
				int ch = sc.nextInt();
				Pers.put("S/N", ch);
			}
			else if(i==2) {
				System.out.println("1. 보험은 들었어?  2. 어떻게 다친 데는 없어?");
				System.out.print("1과 2 중 선택 >>");
				int ch = sc.nextInt();
				Pers.put("T/F", ch);
			}
			else if(i==3) {
				System.out.println("1. 어디가게?정하고 나갈까?  2. 일단 나가서 생각하자!");
				System.out.print("1과 2 중 선택 >>");
				int ch = sc.nextInt();
				Pers.put("J/P", ch);
			}
